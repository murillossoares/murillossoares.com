...
export default function ThemeSwitcher({ label }: { label?: string }) {
  ...
  return (
    <div className="group flex items-center gap-2 bg-panel border border-border px-3 py-2 rounded hover:border-accent transition-colors">
      {label ? <label htmlFor="theme-sel" className="hidden md:inline text-[10px] font-mono text-muted">{label}</label> : null}
      <select id="theme-sel" value={theme ?? "vscode-dark"} onChange={(e) => setTheme(e.target.value)}
